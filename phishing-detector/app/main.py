...
# Import DOM scorer
try:
    from .core.dom_analysis import dom_score
    from .core.visual_analysis import calculate_visual_score
...
ROOT_DIR = os.path.dirname(BASE_DIR) # Parent of app/
MODELS_DIR = os.path.join(ROOT_DIR, "models_data")

# Load URL Model
try:
    model_path = os.path.join(MODELS_DIR, "url_model/hybrid_best_model.keras")
    tokenizer_path = os.path.join(MODELS_DIR, "url_model/tokenizer.pkl")
    scaler_path = os.path.join(MODELS_DIR, "url_model/url_feature_scaler.pkl")
...
# Static files are now in app/static
...
def extract_dom_via_puppeteer(url, output_path="temp_dom.json"):
    # Path to puppeteer script inside app/core
    puppeteer_path = os.path.join(BASE_DIR, "core", "puppeteer_script.js")
    
    try:
        print(f"Running subprocess for URL: {url}")
        # Ensure output path is absolute or handled correctly
...
def get_dom_score(url, brand):
    ...
    if not dom_tree:
        return None, None, brand  # Return brand (might be empty)
    ...
    # So if temp_test_path is "temp_test_dom.json", img is "temp_test_dom.png" (in CWD)
    # We need to move it to our debug dir
    
    # NOTE: Since we run subprocess, CWD is where we ran uvicorn (root). 
    # temp_test_path is likely relative to root.
    root_test_img = temp_test_path.replace('.json', '.png')
    if os.path.exists(root_test_img):
        # Move involves copy-delete or rename. Rename works on same filesystem.
    ...
    if not brand:
        ...
        if isinstance(dom_tree, dict):
            print(f"DEBUG: DOM Tree Keys: {list(dom_tree.keys())}")
            page_title = dom_tree.get("title", "").lower()
        else:
            print("DEBUG: DOM Tree is not a dict (Old format?)")
            page_title = ""
        ...
        if "facebook" in page_title:
            brand = "facebook"
        elif "paypal" in page_title:
            brand = "paypal"
        elif "google" in page_title:
            brand = "google"
        elif "amazon" in page_title:
            brand = "amazon"
        elif "instagram" in page_title:
            brand = "instagram"
        elif "netflix" in page_title:
            brand = "netflix"
        else:
            print("DEBUG: Brand not detected.")
            return None, None, None # Return None brand if detection failed
            
        print(f"DEBUG: Detected Brand: {brand}")
        
    # If we are here, we have a brand (either user-provided or detected)
...
def predict(data: URLRequest):
    try:
        ...
        dom_score, visual_score, detected_brand = get_dom_score(url, brand)
        
        # Update brand if auto-detected
        if not brand and detected_brand:
            brand = detected_brand
            
        # Domain validation (Moved after detection)
        ...
        if brand:
            ...
            is_domain_match = False # No brand to match against
